...
public class SecurityConfig {
    ...
    public KeyPair keyPair() throws Exception {
        ...
        if (StringUtils.hasText(jwtPrivateKey)
                && StringUtils.hasText(jwtPublicKey)) {

            byte[] privateBytes =
                    Base64.getDecoder().decode(jwtPrivateKey);

            byte[] publicBytes =
                    Base64.getDecoder().decode(jwtPublicKey);
        ...
        throw new IllegalArgumentException(
                "Private or public key for JWT is missing"
        );
    }
}